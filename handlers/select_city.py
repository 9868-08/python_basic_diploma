...
def name_get(message):
    if message.text == "Y":
        ...
        bot.set_state(message.from_user.id, bot_states.SelectDates.select_date_in, message.chat.id)
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
def ready_for_answer(message):
    ...
    location_json = rapidapi.get_info.api_request('locations/v3/search', {"q": 'Boston', "locale": "ru_RU"},
